...
export default function Parallelogramm() {
  const [task, setTask] = useState<ParaTask>(() => makeTask());
  ...
  const area = task.base * task.height;
...
function ParallelogramSketch({ base, side, height }: { base: number; side: number; height: number }) {
  ...
  useEffect(() => {
    ...
    drawParallelogram(apiRef.current, base, side, height);
  }, [appletReady, base, side, height]);
...
function drawParallelogram(api: any, base: number, side: number, height: number) {
  if (!api) return;
  try {
    api.reset();
    ...
    api.evalCommand(`B=(${base.toFixed(2)},0)`);
    api.evalCommand(`C=(${(base + offset).toFixed(2)},${height.toFixed(2)})`);
    ...
    api.evalCommand(`gLabel=Text("g = ${format(base)} cm", Midpoint(A,B)+(0,0.4))`);
    api.evalCommand(`sLabel=Text("s = ${format(side)} cm", Midpoint(B,C)+(0.45,0))`);
    api.evalCommand(`hLabel=Text("h = ${format(height)} cm", Midpoint(D,H)+(0.45,0))`);
    api.setColor("gLabel", 15, 118, 178);
    api.setColor("sLabel", 15, 118, 178);
    api.setColor("hLabel", 239, 123, 16);

    const maxDim = Math.max(base + offset, height);
    const pad = maxDim * 0.7;
    api.setCoordSystem(-pad, base + offset + pad, -pad, height + pad);
  } catch (err) {
    console.warn("GeoGebra draw failed", err);
  }
}

function makeTask(): ParaTask {
  const base = randomBetween(10, 18);
  const height = randomBetween(6, 11);
  const offset = randomBetween(base * 0.25, base * 0.65);
  const side = Math.sqrt(offset ** 2 + height ** 2);
