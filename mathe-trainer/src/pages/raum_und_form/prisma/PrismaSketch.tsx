...
export default function PrismaSketch({ a, b, prismHeight, unit }: { a: number; b: number; prismHeight: number; unit: string }) {
  return (
    <svg viewBox="0 0 220 170" className="w-full max-w-[220px] h-44 mx-auto" role="img" aria-label="Dreiecksprisma">
      ...
      <text x="105" y="155" textAnchor="middle" className="fill-slate-600 text-xs">{`a = ${a} ${unit}`}</text>
      <text x="10" y="112" className="fill-slate-600 text-xs">{`b = ${b} ${unit}`}</text>
      <text x="185" y="105" className="fill-slate-600 text-xs">{`H = ${prismHeight} ${unit}`}</text>
    </svg>
  );
}
