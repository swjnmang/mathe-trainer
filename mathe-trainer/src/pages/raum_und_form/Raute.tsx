...
export default function Raute() {
  const [task, setTask] = useState<RhombusTask>(() => makeTask());
  ...
  const area = task.side * task.height;
  ...
  return (
    <TaskLayout
      breadcrumbs={[{ label: "Raum & Form", href: "/raum-und-form" }, { label: "Flächengeometrie", href: "/raum-und-form/flaechengeometrie" }, { label: "Raute" }]}
      title="Raute"
      description="Berechne Flächeninhalt und Umfang einer Raute."
    ...
    >
      <div className={cardClass}>
        ...
        <div className={infoBoxClass}>
          Gegeben ist eine Raute mit Seitenlänge s = {format(task.side)} cm und Höhe h = {format(task.height)} cm. Berechne Flächeninhalt A und Umfang U.
        </div>

        <RhombusSketch side={task.side} height={task.height} />
...
function RhombusSketch({ side, height }: { side: number; height: number }) {
  ...
  useEffect(() => {
    ...
    drawRhombus(apiRef.current, side, height);
  }, [appletReady, side, height]);
...
function drawRhombus(api: any, side: number, height: number) {
  if (!api) return;
  try {
    api.reset();
    ...
    const offset = Math.sqrt(Math.max(side * side - height * height, 0.25));

    api.evalCommand("A=(0,0)");
    api.evalCommand(`B=(${side.toFixed(2)},0)`);
    api.evalCommand(`C=(${(side + offset).toFixed(2)},${height.toFixed(2)})`);
    api.evalCommand(`D=(${offset.toFixed(2)},${height.toFixed(2)})`);
    ...
    api.evalCommand("H=(" + offset.toFixed(2) + ",0)");
    ...
    api.evalCommand(`sLabel1=Text("s = ${format(side)} cm", Midpoint(A,B)+(0,0.4))`);
    api.evalCommand(`sLabel2=Text("s = ${format(side)} cm", Midpoint(B,C)+(0.45,0))`);
    api.evalCommand(`hLabel=Text("h = ${format(height)} cm", Midpoint(D,H)+(0.45,0))`);
    api.setColor("sLabel1", 15, 118, 178);
    api.setColor("sLabel2", 15, 118, 178);
    api.setColor("hLabel", 239, 123, 16);

    const width = side + offset;
...
function makeTask(): RhombusTask {
  return {
    side: randomBetween(8, 14),
