...
export default function ZylinderSketch({ radius, height, unit }: { radius: number; height: number; unit: string }) {
  return (
    <svg viewBox="0 0 220 180" className="w-full max-w-[220px] h-44 mx-auto" role="img" aria-label="Zylinder">
      ...
      <text x="128" y="95" className="fill-slate-600 text-xs">{`h = ${height} ${unit}`}</text>
      <text x="140" y="35" className="fill-slate-600 text-xs">{`r = ${radius} ${unit}`}</text>
    </svg>
  );
}
